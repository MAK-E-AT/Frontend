...
import 'package:makeat_fe/view_models/exercise_select.dart';
...
class _CustomGridSelectState extends State<CustomGridSelect> {
  ...
  List<List<String>> exerciseSelect() {
    List<List<String>> selectedExercise = [];
    for (int i = 0; i < _isSelected.length; i++) {
    if (_isSelected[i]) {
      selectedExercise.add([widget.workoutList[i], widget.workoutInfoList![i]]);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<ExerciseSelectProvider>(
                ...
                setState(() {
                  if (_isSelected[index]) {
                    ...
                    if (widget.workoutInfoList != null) {
                      model.updateSelectedExercise(exerciseSelect());
                    }
                  } else {
